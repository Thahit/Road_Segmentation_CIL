...
from Models.model1_BAYESIAN import get_model
import torch
...
def load_data(train_path, val_path,batch_size, device,alpha):
    ...
    val_dataloader = build_dataloader(val_list,batch_size=batch_size,num_workers=4,device=device,validation=True,alpha=alpha)
...
def visualize_cuda_tensor(image_batch,pred_batch, gt_batch,name):
    image = np.moveaxis((image_batch.detach().cpu().numpy()[:3,:,:] * 128 +128).astype(np.uint8),
                        0, -1)
...
def b_validate(val_dataloader,model,i,pred_thres, device):
    avg_acc = 0
    avg_prec = 0
    avg_rec = 0
    avg_F1 = 0
