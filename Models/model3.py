...
class DownX(nn.Module):
    ...
    def forward(self, x):
        x = self.down_conv(x)
        x = self.tripple(x) + x# residual connections
        return x
...
class UpX(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.pre_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, bias=False)# work with the re-added part
